...
async function findOrCreateContact(contact: CustomerContact, collectionName: string): Promise<string> {
  try {
...
async function findOrCreateProduct(product: Product): Promise<string> {
  try {
...
async function findOrCreatePartner(partner: Partner): Promise<string> {
  try {
...
async function updateCustomer(customerData: CustomerData): Promise<void> {
  try {
      ...
      console.log(`
{
  ...
  "additionalInfo": "Additional information about the customer"
    ...
    const customerData: CustomerData = JSON.parse(jsonData);
...
export { updateCustomer, CustomerData };
